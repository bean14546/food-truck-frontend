<template>
  <div>
    <headerLayout title="จัดการสถานะออเดอร์" name-btn="เพิ่มสถานะออเดอร์" @click="addOrderStatus" />
    <section id="content">
      <v-container class="white rounded-lg pa-6">
        <v-row no-gutters>
          <v-col cols="12" sm="5">
            <v-form @submit.prevent="searchData">
              <v-text-field v-model="search" class="mb-sm-5" prepend-inner-icon="mdi-magnify" label="ค้นหาสถานะออเดอร์" placeholder="กรุณาระบุคำค้นหา" hide-details outlined />
    ...
    <orderListStatusModal ref="orderListStatusModal" />
...
export default {
  ...
  data () {
    return {
      headers: [
        ...
        { text: 'สถานะออเดอร์', align: 'center', value: 'Order_List_Status_Name', width: '15%' },
  ...
  methods: {
    fetchData (pageNumber) {
      this.loading = true
      orderListStatusApi.getAndCount(pageNumber).then((res) => {
    ...
    searchDataOnChangePage (pageNumber) {
      this.loading = true
      if (this.search) {
        orderListStatusApi.search(this.search, pageNumber).then((res) => {
          this.items = res.result.data
          this.$store.commit('setCurrentPage', res.result.meta ? res.result.meta.current_page: res.result.current_page)
          this.perPage = res.result.meta ? res.result.meta.per_page: res.result.per_page
          this.total = res.result.meta ? res.result.meta.total : res.result.total
    ...
    searchData () {
      this.loading = true
      if (this.search) {
        orderListStatusApi.search(this.search).then((res) => {
          this.items = res.result.data
          this.$store.commit('setCurrentPage', res.result.meta ? res.result.meta.current_page: res.result.current_page)
          this.perPage = res.result.meta ? res.result.meta.per_page: res.result.per_page
          this.total = res.result.meta ? res.result.meta.total : res.result.total
