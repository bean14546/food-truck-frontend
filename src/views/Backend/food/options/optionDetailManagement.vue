<template>
  <div>
    <headerLayout title="จัดการ Option Details" name-btn="เพิ่ม Option Details" @click="addOptionDetail" />
    <section id="content">
      <v-container class="white rounded-lg pa-6">
        <v-row no-gutters>
          <v-col cols="12" sm="5">
            <v-form @submit.prevent="searchData">
              <v-text-field v-model="search" class="mb-sm-5" prepend-inner-icon="mdi-magnify" label="ค้นหารายละเอียดของ Option" placeholder="กรุณาระบุคำค้นหา" hide-details outlined />
        ...
        >
          ...
          <template #manage="{ item }">
            <v-btn class="mr-1 mr-sm-0 mr-md-1 rounded-lg" :color="`${!$vuetify.breakpoint.xs} : yellow`" :icon="!$vuetify.breakpoint.xs" :x-small="!$vuetify.breakpoint.xs" :large="$vuetify.breakpoint.xs" elevation="0" :dark="$vuetify.breakpoint.xs" @click="editOptionDetail(item)">
              <v-icon>mdi-pencil</v-icon>
            </v-btn>
            <v-btn class="ml-1 ml-sm-0 ml-md-1 rounded-lg" :color="`${!$vuetify.breakpoint.xs} : primary`" :icon="!$vuetify.breakpoint.xs" :x-small="!$vuetify.breakpoint.xs" :large="$vuetify.breakpoint.xs" elevation="0" :dark="$vuetify.breakpoint.xs" @click="deleteOptionDetail(item)">
...
export default {
  ...
  data () {
    return {
      headers: [
        ...
        { text: 'รายละเอียด Option', align: 'center', value: 'Option_Detail_Name', width: '15%' },
        { text: 'ราคา', align: 'center', value: 'Option_Detail_Price', width: '15%' },
        { text: 'Option', align: 'center', value: 'option', width: '15%' },
  ...
  methods: {
    fetchData (pageNumber) {
      this.loading = true
      optionDetailApi.getAndCount(pageNumber).then((res) => {
    ...
    searchDataOnChangePage (pageNumber) {
      this.loading = true
      if (this.search) {
        optionDetailApi.search(this.search, pageNumber).then((res) => {
          this.items = res.result.data
          this.$store.commit('setCurrentPage', res.result.meta ? res.result.meta.current_page: res.result.current_page)
          this.perPage = res.result.meta ? res.result.meta.per_page: res.result.per_page
          this.total = res.result.meta ? res.result.meta.total : res.result.total
    ...
    searchData () {
      this.loading = true
      if (this.search) {
        optionDetailApi.search(this.search).then((res) => {
          this.items = res.result.data
          this.$store.commit('setCurrentPage', res.result.meta ? res.result.meta.current_page: res.result.current_page)
          this.perPage = res.result.meta ? res.result.meta.per_page: res.result.per_page
          this.total = res.result.meta ? res.result.meta.total : res.result.total
