<template>
  <div>
    <headerLayout title="จัดการหมวดหมู่อาหาร" name-btn="เพิ่มหมวดหมู่อาหาร" @click="addCategory" />
    <section id="content">
      <v-container class="white rounded-lg pa-6">
        ...
        >
          ...
          <template #Category_Image="{ item }">
              <v-img :src="item.Category_Image" contain width="35%" max-width="100px" aspect-ratio="1" />
          </template>
...
export default {
  ...
  data () {
    return {
      headers: [
        ...
        { text: 'รูปภาพ', align: 'start', value: 'Category_Image', width: '10%' },
        { text: 'หมวดหมู่', align: 'start', value: 'Category_Name', width: '15%' },
        { text: 'คำอธิบาย', align: 'start', value: 'Category_Description', width: '25%' },
        { text: 'สร้างเมื่อ', align: 'center', value: 'created_at', width: '10%' },
  ...
  methods: {
    fetchData (pageNumber) {
      this.loading = true
      categoryApi.getAndCount(pageNumber).then((res) => {
    ...
    searchDataOnChangePage (pageNumber) {
      this.loading = true
      if (this.search) {
        categoryApi.search(this.search, pageNumber).then((res) => {
          this.items = res.result.data
          this.$store.commit('setCurrentPage', res.result.meta ? res.result.meta.current_page : res.result.current_page)
          this.perPage = res.result.meta ? res.result.meta.per_page : res.result.per_page
          this.total = res.result.meta ? res.result.meta.total : res.result.total
    ...
    searchData () {
      this.loading = true
      if (this.search) {
        categoryApi.search(this.search).then((res) => {
          this.items = res.result.data
          this.$store.commit('setCurrentPage', res.result.meta ? res.result.meta.current_page : res.result.current_page)
          this.perPage = res.result.meta ? res.result.meta.per_page : res.result.per_page
          this.total = res.result.meta ? res.result.meta.total : res.result.total
